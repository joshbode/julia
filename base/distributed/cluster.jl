...
`start_worker(cookie)` is equivalent to `start_worker(STDOUT, cookie)`.
If the cookie is an empty string, `""`, the worker tries to read the cookie from its STDIN.

The function redirects STDERR to STDOUT, listens on a free port (unless
the port is explictly specified by the `--bind-to` command line option) and schedules tasks to
process incoming TCP connections and requests. It does not return.
"""
